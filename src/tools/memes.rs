...
pub fn register(registry: &mut ToolRegistry, config: AppConfig, paths: MiyuPaths) {
    ...
    // 增/改/删合并成 `manage_meme`(08-17):同一份表情库的三种写操作。
    // 检索与发送(search_meme/show_meme)是读路径,常驻不动。
    registry.register(
        ToolSpec::new(
            "manage_meme",
            t(
                "Maintain the current persona's writable meme library. action=add imports a local image (metadata is generated by the configured vision model when not supplied); action=update edits index metadata in the writable overlay; action=delete removes a user meme or disables a built-in one.",
                "维护当前人格的可写表情库。action=add 导入本地图片（未提供元数据时由配置的识图模型生成）；action=update 修改可写覆盖层里的元数据；action=delete 删除用户表情或禁用内置表情。",
            ),
            json!({
                "type": "object",
                "properties": {
                    "action": { "type": "string", "enum": ["add", "update", "delete"], "description": t("add imports, update edits, delete removes.", "add 导入，update 修改，delete 删除。") },
                    "image": { "type": "string", "description": t("action=add: local image path.", "action=add：本地图片路径。") },
                    "id": { "type": "string", "description": t("action=update/delete: full sha256 id or unique short id.", "action=update/delete：完整 sha256 id 或唯一短 id。") },
                    ...
                    "enabled": { "type": "boolean", "description": t("action=update: enable or disable this meme.", "action=update：启用或禁用该表情。") },
                    "hard_delete": { "type": "boolean", "description": t("action=delete: permanently remove the user image instead of moving it to trash.", "action=delete：永久删除用户图片，而不是移入回收站。") }
                },
                "required": ["action"],
                ...
                move |args| {
                    ...
                    async move {
                        match args.get("action").and_then(Value::as_str).unwrap_or_default() {
                            ...
                            other => bail!("unknown action: {other}; expected add, update or delete"),
                        }
                    }
                }
            },
